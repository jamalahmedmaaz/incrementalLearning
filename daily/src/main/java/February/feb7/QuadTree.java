...
public class QuadTree<Key extends Comparable<Key>, Value> {
    ...
    public static void main(String[] args) {
        int M = Integer.parseInt("4");   // queries
        int N = Integer.parseInt("4");   // points
    ...
    private void query2D(Node h, Interval2D<Key> rect) {
        ...
        Key xmin = (Key) rect.intervalX.min();
        Key ymin = (Key) rect.intervalY.min();
        Key xmax = (Key) rect.intervalX.max();
        Key ymax = (Key) rect.intervalY.max();
    ...
    private static class Interval<Key> {
        Key xmin;
        Key xmax;

        public Interval(Key xmin, Key xmax) {
            this.xmax = xmax;
            this.xmin = xmin;
        }
    ...
    private static class Interval2D<Key> {
        public Interval intervalX;
        public Interval intervalY;

        public Interval2D(Interval<Key> intX, Interval<Key> intY) {
            this.intervalX = intX;
            this.intervalY = intY;
        }
    ...
    class ExampleOfQuadTree {
        ...
        private Collection<Object> searchBusinesses(QuadTreeNode node) {
            return null;
        }

        class QuadTreeNode {
            List<QuadTreeNode> childrens;
