...
public class CompleteDyanmicProgramming {
    ...
    class LongestValidParentheses {
         ...
         * the string scanned so far is valid, and also the length of the longest
         * valid string. In order to do so, we start by pushing -1−1 onto the stack.
         * <p>
         * For every ‘(’ encountered, we push its index onto the stack.
         * <p>
         * For every ‘)’ encountered, we pop the topmost element and
         * subtract the current element's index from the top element of the
         * stack, which gives the length of the currently encountered valid
         * string of parentheses. If while popping the element, the stack
         * becomes empty, we push the current element's index onto the stack.
         * In this way, we keep on calculating the lengths of the valid
         ...
         * @return
         */
        public int longestValidParentheses_Using_Stack(String s) {
            int maxans = 0;
                ...
                if (tmp.length() == 0) {
                    ...
                    for (String word : words) {
                        if (tmp.startsWith(word)) {
                            result = result | rec(tmp.substring(word.length()));
                        }
                    }
                    map.put(tmp, result);
                    return result;
